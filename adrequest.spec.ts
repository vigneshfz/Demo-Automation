...
//ENTERING SHIPPING BILL NUMBER:
 ...
 const[uploadFiles] = await Promise.all([
  ...
  await page.frameLocator('iframe[title="widgetInIframe"]').getByRole('textbox').click()
 // await page.locator('div').filter({ hasText: /^cloud_uploadDrag and Drop Files$/ }).first().click()
  //await page.frameLocator('iframe[title="widgetInIframe"]').locator('div').filter({ hasText: /^cloud_uploadDrag and Drop Files$/ }).first().click()
// await page.click('//*[@id="fileDropRef"]')
]);

  uploadFiles.setFiles("tests/uploadFiles/demo.xlsx");
 //uploadFiles.setFiles('tests/uploadFile/dem.xls');
 uploadFiles.setFiles('./csv .csv');
 uploadFiles.setFiles('tests/uploadFiles/Docs.docx');
 await page.waitForTimeout(5000);
 uploadFiles.setFiles('Execution count.pdf');
// uploadFiles.setFiles('Execution count-FCC.pdf');
uploadFiles.setFiles('123.jpeg')
...
page.frameLocator('iframe[title="widgetInIframe"]').getByRole('button', { name: 'Choose' }).click(),
 ...
 uploadFiles.setFiles(['dem.xlsx']);
  ...
  await page.frameLocator('iframe[title="widgetInIframe"]').getByLabel('Title *').fill('sample');
